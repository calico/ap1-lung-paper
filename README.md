...
This repository contains the scripts used to process and analyze the single-cell multiome, bulk RNA-seq, ATAC-seq, and published human datasets reported in the paper, and to generate the figures.
...
### Running the scripts
Most of the repository is R. The scripts are not organized as an R package; each is a standalone script meant to be run section by section in an R session (or with `Rscript`). Nothing is installed from this repository — the packages the scripts depend on are installed from CRAN, Bioconductor, and GitHub. The one exception is `atac/atac_process.sh`, a shell script that processes ATAC-seq reads on a Slurm cluster.

The scripts use placeholder paths (e.g. `/path/to/...`). To reproduce an analysis, set these to the location of the corresponding data on your system. Inputs for each script are listed at the top of that script.

### Command-line tools
ATAC-seq read processing (`atac/atac_process.sh`) was run on a Slurm cluster using cutadapt (2.8.0), bowtie2 (v2.3.4), and samtools (v1.14), aligning to mm10.
...
- **`atac/`** — bulk ATAC-seq
  - `atac_process.sh` — Slurm array script to process ATAC fastq files to sorted BAM files (cutadapt → bowtie2 → samtools)
  - `ChrAccR.R` — accessibility analysis and differential accessibility from the BAMs

### Dependencies between scripts

Most analyses are contained within their experimental folder. However, a few analyses depend on objects produced elsewhere in the repo: `bulk_rnaseq/KD_OE.R` reads the fibroblasts_final RDS object in the GEO repository, and `bulk_rnaseq/inhibitor_screen.R` uses the AFOS sample table and counts matrix.
